...
class TrainModel():
	...
	if self.param['question_type']=='all':
		self.param['question_type']=','.join(self.read_data.all_questypes_inv.keys())
    ...
    def forward_pass_interpreter(self, batch_orig_context, a_seq, per_step_probs, \
		...
		try:
			print 'Rule Based\'s Query ',self.rule_based_logs[data_ids[batch_id]]['query']
			print 'Rule Based Program ',self.rule_based_logs[data_ids[batch_id]]['program']
			print 'Rule Based Program\'s Jaccard ', self.rule_based_logs[data_ids[batch_id]]['jaccard'] 
    ...
    def train(self):
		...
		#train_data = self.rectify_ques_type(train_data)
