...
class NPI(nn.Module):
    def __init__(self, params, none_argtype_index, num_argtypes, num_programs,
                 max_arguments,
                 rel_embedding, rel_date_embedding, vocab_embed,
        ...
        self.params = params
        self.use_kb_emb = params['use_kb_emb']
        self.num_timesteps = params['num_timesteps']
        self.max_num_phase_1_steps = math.ceil(self.num_timesteps/2.0)
        ...
        self.wikidata_embed_dim = params['freebase_embed_dim']
        ...
        def create_cell_scopes():
            ...
            for param in self.npi_rnn.parameters:
            ...
            for param in self.env_rnn.parameters:
                param.requires_grad = False
            ...
            if self.use_kb_emb:
                ...
                self.sentence_rnn = nn.GRU(input_size=self.text_embed_dim, \
                                       hidden_size=self.cell_dim, batch_first=True)
            for param in self.sentence_rnn.parameters:
                param.requires_grad = False
            self.reset_scope = 'reset_scope'
            self.reset_layer = nn.Linear(self.npi_core_dim, self.npi_core_dim)
            self.reset_layer.weight.requires_grad = False
            self.reset_layer.bias.requires_grad = False
            ...
            self.state_encoding_layer1.weight.requires_grad = False
            self.state_encoding_layer1.bias.requires_grad = False
            ...
            self.state_encoding_layer2.weight.requires_grad = False
            self.state_encoding_layer2.bias.requires_grad = False
            ...
            self.state_encoding_layer3.weight.requires_grad = False
            self.state_encoding_layer3.bias.requires_grad = False

            self.phase_change_scope = 'phase_change_scope'
            self.phase_change_layer = nn.Linear(self.npi_core_dim, 1)
            self.phase_change_layer.weight.requires_grad = False
            self.phase_change_layer.bias.requires_grad = False
            ...
            self.prog_key_layer1.weight.requires_grad = False
            self.prog_key_layer1.bias.requires_grad = False

            self.prog_key_scope2 = 'prog_net_fcc2'
            self.prog_key_layer2 = nn.Linear(self.prog_key_dim, self.prog_key_dim)
            self.prog_key_layer2.weight.requires_grad = False
            self.prog_key_layer2.bias.requires_grad = False
            ...
            rel_date_embedding_mat = torch.tensor(rel_date_embedding, device=self.device)
            self.rel_date_embedding = nn.Embedding(rel_date_embedding.shape[0], rel_date_embedding.shape[1], _weight=rel_date_embedding_mat)
...
#                    beam_properties['check_penalization'][beam_id] = to_penalize_beams[beam_id]
                    ...
                    current_beam_score[beam_id] = torch.clamp(current_beam_score[beam_id],4*math.log(self.eps),0)
...
#                flag_penalize = torch.min(to_penalize_score,dim=1,keepdim=True)[0]
                ...
                current_score = torch.clamp(current_score,4*math.log(self.eps),0)
...
#        print num_variables_remaining[0].dtype,program_to_targettype_onehot.dtype
	...
	#print 'variable mask ', self.variable_mask[0]
	...
	#print 'num_variable_types ', num_variable_types[0]
        #num_variable_types is a beam_size sized list of dimension batch_size x num_progs x num_argtypes
	#print 'self.program_to_num_arguments ', self.program_to_num_arguments
	...
	#print 'feasible_progs 1', feasible_progs[0]
	...
	#print 'feasible_progs 2', feasible_progs[0]
	...
	#print 'program_to_kb_attention ', program_to_kb_attention
	#print 'feasible_progs 3', feasible_progs[0]
	...
	#print 'feasible_progs 4', feasible_progs[0]
