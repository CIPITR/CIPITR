...
for line in open(sys.argv[3]).readlines():
	...
	if validation_going_on:
		if 'Validation over' in line:
			...
			continue	
	if not (line.startswith('batch id') and ':: Query ::' in line) and not line.startswith('beam id') and not line.startswith('In calculate') and not 'best reward till this beam' in line and not 'training is completed ... overall avg. train loss' in line and not '(avg over batch) train loss' in line:
                continue
	...
	if '(avg over batch) train loss' in line:
                #print len(running_avg_reward)
		if len(running_avg_reward)==16:
			#print '[[OLD]] ', line.strip()
			print '[[NEW]] (avg over batch) train loss ',batch_reward, 'running avg over batches (best over all) ', float(sum(running_avg_reward))/16.0, '  and (top beam) ',  float(sum(running_avg_reward_topbeam))/16.0
			running_avg_reward = []
			running_avg_reward_topbeam = []
			
	if 'training is completed' in line:
		#print '[[OLD]] ', line.strip()
		epoch = line.split(' ')[2]
		print '[[NEW]]  epoch ', epoch, 'training is completed ... overall avg. train reward (best over all) ', float(avg_reward_over_epoch)/float(num_batches), ' avg. train reward (top beam) ', float(avg_reward_over_epoch_topbeam)/float(num_batches)
		avg_reward_over_epoch = 0.0
		avg_reward_over_epoch_topbeam = 0.0
		num_batches = 0
		print ''
