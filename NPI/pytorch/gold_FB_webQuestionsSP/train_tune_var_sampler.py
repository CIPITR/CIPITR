...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov  7 20:59:45 2018

@author: Ahmed Ansari
@email: ansarighulamahmed@gmail.com
"""

from model_tune_var_sampler import NPI
...
class TrainModel():
    def __init__(self, param):
        ...
        self.optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, self.model.parameters()), lr=learning_rate, betas=[0.9,0.999], weight_decay=1e-5)
        print self.model
    ...
    def forward_pass_interpreter(self, batch_orig_context, a_seq, per_step_probs, \
                                 ...
                                 batch_response_dates, epoch_number, overall_step_count, kb_attention):
    ...
    def get_data_and_feed_dict(self, batch_dict, epoch, overall_step_count):
        ...
        return feed_dict1, batch_orig_context, batch_response_entities, batch_response_dates, variable_value_table

    def perform_validation(self, batch_dict, epoch, overall_step_count):
        self.model = self.model.eval()
        with torch.no_grad():
            feed_dict1, batch_orig_context, batch_response_entities, batch_response_dates, variable_value_table = self.get_data_and_feed_dict(batch_dict, epoch, overall_step_count)
            ...
            [reward, intermediate_rewards, relaxed_rewards, intermediate_rewards_flag, \
             mask_intermediate_rewards] = self.forward_pass_interpreter(batch_orig_context, a_seq, per_step_probs, \
                                             program_probabilities, variable_value_table, batch_response_entities, \
                                             batch_response_dates, epoch, overall_step_count, feed_dict1['kb_attention'])
