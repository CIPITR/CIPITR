...
class ReadBatchData():
    ...
    def get_response_type(self, question):
        """
        This method predicts the variable type of the answer, based on a heuristic model on the query wording
        """
    ...
    def get_data_per_questype(self, data_dict):
        """
        This method creates a dictionary of data subsets corresponding to each question type
        """
        data_dict_questype = {}
        for d in data_dict:
            state = d[11]
    ...
    def get_all_required_argtypes(self, type, reqd_argtypes):
        """
        This method creates the list of variable argument types required 
        to be generated in a program in order to create an answer of the desired (predicted) variable type
        """
    ...
    def get_all_required_argtypes_matrix(self, reqd_argtypes):
        """
        converts the dictionary of the list of required variable types (to create an answer variable of the desired type) into a numpy matrix
        """
    ...
    def get_batch_data(self, data_dict):
        """
        this method preprocesses the data read in batches from the pkl file
        """
        ...
        for i in range(num_data):
            ...
            if batch_questype[i]=='verify':
                ...
                if len(batch_response_entities[i])>0:
                    batch_response_entities[i] = []
        if len(data_dict[i])==16:
            batch_rel_attention = [data_dict[i][12] for i in range(num_data)]
            batch_type_attention = [data_dict[i][13] for i in range(num_data)]
            batch_ent_rel_type_kb_subgraph = [data_dict[i][14] for i in range(num_data)]
            batch_type_rel_type_kb_subgraph = [data_dict[i][15] for i in range(num_data)]
        else:
    ...
    def debug_kb_attention(self, kb_attention, arg1, arg2, arg3, vocab1, vocab2, vocab3, type):
        """
        This method is used for debugging the kb consistency matrix by printing it
        """
    ...
    def get_variable_value_table(self, entity_variable_value_table, rel_variable_value_table, type_variable_value_table, int_variable_value_table):
        """
        This method is used for creating the variable value table i.e. a matrix for recording the prepopulated memory table storing the values of the memory variables
        """
    ...
    def get_kb_attention(self, batch_ent_rel_type_kb_subgraph, batch_type_rel_type_kb_subgraph):
        """
        This method is used for constructing the matrix for storing the KB consistency scores for each variable instantiation for each operator
        """
    ...
    def get_variable_table_data(self, batch_context_entities, batch_context_relations, batch_context_types, \
        ...
        """
        this method is used for constructing the memory related matrices for the NPI model; 
        variable mask for storing a boolean mask value indicating whether the memory location is occupied by a valid variable or is empty
        variable_embed for storing the embedding vector of the variables prepopulated in the memory
        variable_atten for storing the attention distribution over the variables of each type in the memory
        """
