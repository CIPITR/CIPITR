...
for line in open(sys.argv[3]).readlines():
	...
	if '(avg over batch) train loss' in line:
		...
		if len(running_avg_reward)==50:
			#print '[[OLD]] ', line.strip()
			print '[[NEW]] (avg over batch) train loss ',batch_reward, 'running avg over batches (best over all) ', float(sum(running_avg_reward))/50.0, '  and (top beam) ',  float(sum(running_avg_reward_topbeam))/50.0
			running_avg_reward = []
			running_avg_reward_topbeam = []
