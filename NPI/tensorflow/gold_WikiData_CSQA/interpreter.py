...
class Interpreter():
    def __init__(self, wikidata_dir, num_timesteps, program_type_vocab, argument_type_vocab, printing, relaxed_reward_strict, reward_function="jaccard", boolean_reward_multiplier = 0.1, relaxed_reward_till_epoch=[-1,-1], unused_var_penalize_after_epoch=[1000, 100000], length_based_penalization=False):
        """
    ...
    def calculate_program_reward(self, args, epoch_number, overall_step_number):
        """
        This method unpacks the input argument and invokes the below method on these arguments
        """
        if self.parallel == 1:
    ...
    def execute_multiline_program(self,inputs, gold_entities, gold_ints, gold_bools, epoch_number, overall_step_number):
        ...
        for i in range(self.num_timesteps):
            ...
            if 'terminate' in self.program_type_vocab and program_type_id == self.program_type_vocab['terminate']:
                ...
                break
            if program_type_id != self.program_type_vocab['none']:
